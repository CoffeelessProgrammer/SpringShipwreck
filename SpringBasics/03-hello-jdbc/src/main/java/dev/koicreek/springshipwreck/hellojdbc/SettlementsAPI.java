...
public class SettlementsAPI implements InitializingBean {
    ...
    @GetMapping
    ResponseEntity<List<SettlementCM>> getSettlements() {
        List<SettlementCM> result = new ArrayList<>();

        try (Connection con = dataSource.getConnection(); Statement statement = con.createStatement()) {
            try (ResultSet settlements = statement.executeQuery("SELECT * FROM SETTLEMENTS")) {
                while(settlements.next()) {
                    result.add(new SettlementCM(settlements.getInt("city_id"),
        ...
        return new ResponseEntity<List<SettlementCM>>(result, HttpStatus.OK);

    }

    @GetMapping("/{id}")
    ResponseEntity<SettlementCM> getSettlementById(@PathVariable int id) {
        SettlementCM result = null;
        ...
        try (Connection con = dataSource.getConnection();
            ...
            try (ResultSet settlements = statement.executeQuery()) {
                if(settlements.next()) {
                    result = new SettlementCM(settlements.getInt("city_id"),
                            settlements.getString("city_name"),
                            settlements.getString("region"),
                            settlements.getString("slogan"));
                }
        ...
        if(result == null)
            return new ResponseEntity<SettlementCM>(HttpStatus.NOT_FOUND);

        return new ResponseEntity<SettlementCM>(result, HttpStatus.OK);
    }

    @PostMapping
    ResponseEntity<Map<String, Integer>> addSettlement(@RequestBody @Valid SettlementCM request) {
        try (Connection con = dataSource.getConnection()) {
